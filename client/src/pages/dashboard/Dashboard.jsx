...
const Dashboard = () => {
    ...
    return !loading ? (
        <div className='h-screen relative'>
            ...
            {option === 3 && <MoneyTransfer ref={newRef} refreshUserData={refreshUserData} />}
            ...
            <div className='bg-[#E5E6ED] h-[78%] flex flex-row gap-x-20 items-top pt-10 pl-20'>
                <div className='w-[500px] bg-white h-[90%] rounded-md font-mono p-4 shadow-lg'>
                    ...
                    <div className='p-2 flex flex-col'>
                        {user?.accounts?.map((account, idxacc) =>
                            account.logs.map((log, idxlog) => (
                                <li key={idxlog + "" + idxacc} className='text-lg w-full text-left'>
                                    {log}
                                </li>
                            ))
                        )}
